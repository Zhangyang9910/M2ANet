...
from .networks import M2ANet
from .loss import AdversarialLoss, PerceptualLoss, StyleLoss, Contrastive, UnetDiscriminator
...
class BaseModel(nn.Module):
    ...
    def save(self):
        ...
        torch.save({
            'iteration': self.iteration,
            'generator': self.generator.state_dict()
        }, os.path.join(self.config.PATH, str(self.iteration).zfill(7) + '_gen.pth'))
...
class InpaintingModel(BaseModel):
    def __init__(self, config):
        ...
        self.add_module('contrastive_loss', contrastive_loss)
        self.add_module('contrastive_loss_learning', contrastiveloss)
    ...
    def process(self, images, masks):
        ...
        _, _, _, dis_real = self.discriminator(dis_input_real)
        _, _, _, dis_fake = self.discriminator(dis_input_fake)
        ...
        # contrastive learning losses
        edge_loss, semantic_loss = self.contrastive_loss(images_masked, outputs_img, images, masks, 5)
        edge_loss = edge_loss * self.config.INPAINT_textural_LOSS_WEIGHT
        semantic_loss = semantic_loss * self.config.INPAINT_semantic_LOSS_WEIGHT
        contrastive_loss = edge_loss + semantic_loss
        dis_loss += contrastive_loss

        # generator adversarial loss
        gen_input_fake = outputs_img
        _, _, _, gen_fake = self.discriminator(gen_input_fake)
        ...
        return outputs_img, gen_loss, dis_loss, logs, gen_gan_loss, gen_l1_loss, gen_content_loss, gen_style_loss, contrastive_loss
