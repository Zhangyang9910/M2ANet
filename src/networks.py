...
class CBAM(nn.Module):
    def __init__(self, dim, dilation, reduction_ratio=16, kernel_size=7):
        super(CBAM, self).__init__()

        # 通道注意力和空间注意力
        self.channel_attention = ChannelAttention(dim, reduction_ratio=reduction_ratio)
        self.spatial_attention = SpatialAttention(kernel_size=kernel_size)

        self.conv = nn.Sequential(
            # nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=True),
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=dilation, dilation=dilation, bias=True),
    ...
    def forward(self, x):
        out1 = self.conv(x)
        # 通道注意力
        out = self.channel_attention(x) * out1

        out2 = self.conv(out1)

        # 空间注意力
        out = self.spatial_attention(out) * out2
...
class SandwichBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, dilation):
...
##########################################################################
## Down-Sampling (DMD)
class DownsampleWithDepthwiseSeparable(nn.Module):
    def __init__(self, n_feat, depth_multiplier=1):
        super(DownsampleWithDepthwiseSeparable, self).__init__()

        self.depthwise_conv_feat = nn.Sequential(
            nn.Conv2d(n_feat, n_feat, kernel_size=3, stride=2, padding=1, bias=False),
            nn.Conv2d(
                in_channels=n_feat,
                out_channels=n_feat * depth_multiplier,
                kernel_size=3,
                stride=1,
                padding=1,
                groups=n_feat))

        self.pointwise_conv_feat = nn.Conv2d(
            in_channels=n_feat * depth_multiplier,
        ...
        self.depthwise_conv_mask = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1, bias=False),
            nn.Conv2d(
                in_channels=1,
                out_channels=1 * depth_multiplier,
                kernel_size=3,
                stride=1,
                padding=1,
                groups=1))

        self.pointwise_conv_mask = nn.Conv2d(
            in_channels=1 * depth_multiplier,
            out_channels=4,
            kernel_size=1)
    ...
    def forward(self, x, mask):
        ...
        depthwise_x = self.depthwise_conv_feat(x)

        out = self.pointwise_conv_feat(depthwise_x)

        depthwise_mask = self.depthwise_conv_mask(mask)

        out_mask = self.pointwise_conv_mask(depthwise_mask)
    ...
    def __init__(self,
        ...
        self.encoder_level1 = nn.Sequential(*[
            SandwichBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                          bias=bias, dilation=dilation[0],
                          LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.down1_2 = DownsampleWithDepthwiseSeparable(dim)
        self.encoder_level2 = nn.Sequential(*[
            SandwichBlock(dim=int(dim * 2 ** 1), num_heads=heads[1],
                          ffn_expansion_factor=ffn_expansion_factor, dilation=dilation[1],
                          bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.down2_3 = DownsampleWithDepthwiseSeparable(int(dim * 2 ** 1))
        self.encoder_level3 = nn.Sequential(*[
            SandwichBlock(dim=int(dim * 2 ** 2), num_heads=heads[2],
                          ffn_expansion_factor=ffn_expansion_factor, dilation=dilation[2],
                          bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])

        self.down3_4 = DownsampleWithDepthwiseSeparable(int(dim * 2 ** 2))
        self.latent = nn.Sequential(*[
            SandwichBlock(dim=int(dim * 2 ** 3), num_heads=heads[3],
                          ffn_expansion_factor=ffn_expansion_factor, dilation=dilation[3],
        ...
        self.decoder_level1 = nn.Sequential(*[
            SandwichBlock(dim=int(dim * 2 ** 1), num_heads=heads[0],
                          ffn_expansion_factor=ffn_expansion_factor, dilation=dilation[0],
